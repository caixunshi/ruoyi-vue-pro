    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button type="primary" plain icon="el-icon-plus" size="mini" @click="handleAdd"
                   v-hasPermi="['mp:employee:create']">新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="warning" plain icon="el-icon-download" size="mini" @click="handleExport" :loading="exportLoading"
                   v-hasPermi="['mp:employee:export']">导出</el-button>
      </el-col>
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template v-slot="scope">
          <el-button size="mini" type="text" icon="el-icon-edit" @click="handleUpdate(scope.row)"
                     v-hasPermi="['mp:employee:update']">修改</el-button>
          <el-button size="mini" type="text" icon="el-icon-delete" @click="handleDelete(scope.row)"
                     v-hasPermi="['mp:employee:delete']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <!-- 分页组件 -->
...
<script>
import { createEmployee, updateEmployee, deleteEmployee, getEmployee, getEmployeePage, exportEmployeeExcel } from "@/api/mp/employee";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      // 执行查询
      getEmployeePage(this.queryParams).then(response => {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除员工信息编号为"' + id + '"的数据项?').then(function() {
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有员工信息数据项?').then(() => {
