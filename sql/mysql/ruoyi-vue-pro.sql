...
INSERT INTO `system_role` (`id`, `name`, `code`, `sort`, `data_scope`, `data_scope_dept_ids`, `status`, `type`, `remark`, `creator`, `create_time`, `updater`, `update_time`, `deleted`, `tenant_id`) VALUES (101, '测试账号', 'test', 0, 1, '[]', 0, 2, '132', '', '2021-01-06 13:49:35', '1', '2022-09-25 12:09:38', b'0', 1);
...
) ENGINE = InnoDB AUTO_INCREMENT = 484 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = '手机验证码';
...
CREATE TABLE `system_sms_log`  (
  ...
  `deleted` bit(1) NOT NULL DEFAULT b'0' COMMENT '是否删除',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 349 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = '短信日志';
...
CREATE TABLE `zabank_service`.`Untitled`  (
                                              ...
                                              PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB AUTO_INCREMENT = 1 CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '门店信息表' ROW_FORMAT = Dynamic;

ALTER TABLE `zabank_service`.`t_store_info`
    ADD COLUMN `creator` varchar(64) NOT NULL COMMENT '创建人' AFTER `latitude`,
ADD COLUMN `create_time` datetime(0) NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间' AFTER `creator`,
ADD COLUMN `updater` varchar(64) NULL COMMENT '修改人' AFTER `create_time`,
ADD COLUMN `update_time` datetime(0) NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '修改时间' AFTER `updater`,
ADD COLUMN `deleted` bit(1) NOT NULL DEFAULT b'0' COMMENT '删除标识' AFTER `update_time`,
ADD COLUMN `tenant_id` bigint(20) NOT NULL COMMENT '租户Id' AFTER `deleted`;